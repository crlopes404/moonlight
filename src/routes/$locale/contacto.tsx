...
export const Route = createFileRoute("/$locale/contacto")({
  head: () => localizedHead({ path: "contacto", title: m.meta_contacto_title(), description: m.meta_contacto_desc() }),
  component: Contacto,
});

const services = [
  { id: "dev", label: () => "Software Development" },
  { id: "bi", label: () => "BI & Marketing" },
  { id: "consulting", label: () => "IT Consulting" },
  { id: "iot", label: () => "IoT / AR / VR" },
  { id: "sla", label: m.ct_svc_sla },
  { id: "banalytics", label: () => "B.Analytics" },
  { id: "other", label: m.ct_svc_other },
];

const faqs = [
  { q: m.ct_faq1_q, a: m.ct_faq1_a },
  { q: m.ct_faq2_q, a: m.ct_faq2_a },
  { q: m.ct_faq3_q, a: m.ct_faq3_a },
  { q: m.ct_faq4_q, a: m.ct_faq4_a },
  { q: m.ct_faq5_q, a: m.ct_faq5_a },
];
    ...
    <>
      ...
      <section className="relative pt-32 md:pt-44 pb-12 md:pb-16 overflow-hidden">
        ...
        <div className="mx-auto max-w-3xl px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="eyebrow">{m.ct_hero_eyebrow()}</motion.div>
          <motion.h1
          ...
          >
            {m.ct_hero_t1()} <span className="text-gradient italic font-light">{m.ct_hero_accent()}</span>.
          </motion.h1>
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            {m.ct_hero_p1()}
            <span className="text-foreground font-medium"> {m.ct_hero_p_accent()}</span>.
          </motion.p>
          ...
          >
            <button data-magnetic onClick={scrollToForm} className="btn-magnetic btn-primary">
              <span className="inline-flex items-center gap-2">{m.ct_btn_start()} <ArrowRight className="size-4" /></span>
            </button>
            ...
            <div className="relative grid lg:grid-cols-[1.2fr_auto] gap-10 items-center">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full glass px-3.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.25em] text-primary">
                  <Sparkles className="size-3.5" /> {m.ct_brief_badge()}
                </div>
                <h2 className="mt-5 font-display text-[clamp(1.9rem,4vw,3rem)] leading-[1.05] tracking-tight">
                  {m.ct_brief_t1()}<br />
                  <span className="text-gradient">{m.ct_brief_t2()}</span>
                </h2>
                ...
                <ul className="mt-7 flex flex-wrap gap-x-8 gap-y-3 text-sm text-muted-foreground">
                  <li className="inline-flex items-center gap-2"><Clock className="size-4 text-primary" /> {m.ct_brief_li1()}</li>
                  <li className="inline-flex items-center gap-2"><ShieldCheck className="size-4 text-primary" /> {m.ct_brief_li2()}</li>
                  <li className="inline-flex items-center gap-2"><FileText className="size-4 text-primary" /> {m.ct_brief_li3()}</li>
                </ul>
              </div>
      ...
      <section id="form" className="relative section-sm scroll-mt-28">
        <div className="mx-auto max-w-3xl px-6">
          <motion.form
            ...
            onSubmit={(e) => { e.preventDefault(); alert(m.ct_alert()); }}
          ...
          >
            ...
            <div className="relative">
              <div className="eyebrow">{m.ct_form_eyebrow()}</div>
              <h2 className="mt-3 font-display text-3xl md:text-4xl tracking-tight">{m.ct_form_title()}</h2>
              <p className="mt-3 text-muted-foreground">{m.ct_form_sub()}</p>

              <div className="mt-9 grid sm:grid-cols-2 gap-5">
                <Field label={m.ct_field_name()} id="name" delay={0} />
                <Field label={m.ct_field_company()} id="company" delay={0.05} />
                <Field label={m.ct_field_email()} id="email" type="email" delay={0.1} />
                <Field label={m.ct_field_phone()} id="phone" required={false} delay={0.15} />
              </div>

              <div className="mt-8">
                <label className="font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground">{m.ct_services_label()}</label>
              ...
              <div className="mt-8">
                <label htmlFor="msg" className="font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground">{m.ct_msg_label()}</label>
                <textarea
                  id="msg"
                  rows={5}
                  required
              ...
              <button data-magnetic type="submit" className="btn-magnetic btn-primary mt-9 w-full sm:w-auto justify-center">
                <span style={{ transform: "translateZ(20px)" }} className="inline-flex items-center gap-2">
                  {m.ct_submit()} <ArrowRight className="size-4" />
                </span>
              </button>
            </div>
          </motion.form>
      ...
      <section className="relative section">
        <div className="mx-auto max-w-4xl px-6">
          <SectionHeader eyebrow="FAQ" title={m.ct_faq_title()} />
      ...
      <section className="relative section-sm text-center">
        <div className="mx-auto max-w-3xl px-6">
          <h3 className="font-display text-4xl md:text-5xl tracking-tight">
            {m.ct_email_cta_1()} <a href="mailto:suporte@moonlight.pt" data-magnetic className="text-gradient">{m.ct_email_cta_link()}</a>.
          </h3>
        </div>
      </section>
    </>
